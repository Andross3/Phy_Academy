...
import TareaRestricciones from './tareaRestricciones';
import TareaPlantilla from './tareaPlantilla';
...
export default function FormularioTarea({ id_tema, onSuccess, onCancel, modo = 'modal', initialData }) {
  const [titulo, setTitulo] = useState(initialData?.titulo || '');
  const [descripcion, setDescripcion] = useState(initialData?.descripcion || '');
  const [tipoTarea, setTipoTarea] = useState(initialData?.tipo_tarea || '');
  const [open, setOpen] = useState(false);
  const selectRef = useRef(null);
  const [restricciones, setRestricciones] = useState(initialData?.restricciones || []);
  const [codigo, setCodigo] = useState(initialData?.codigo_plantilla || '');
  const [fechaEntrega, setFechaEntrega] = useState(initialData?.fecha_entrega || '');
  const [horaEntrega, setHoraEntrega] = useState(initialData?.hora_entrega || '');
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setTipoTarea(initialData.tipo_tarea || '');
      setRestricciones(initialData.restricciones || []);
      setCodigo(initialData.codigo_plantilla || '');
  ...
  const enviarDatos = async () => {
    ...
    if (id_tema) datos.id_tema = id_tema;
    try {
      let response;
      ...
      if (response.ok) {
        mostrarModal(initialData ? 'La tarea se ha editado exitosamente.' : 'La tarea se ha creado exitosamente.');
        limpiarFormulario();
        if (onSuccess) onSuccess();
      } else {
        const errorData = await response.json().catch(() => ({}));
        mostrarModal(
          errorData.message || (initialData ? 'Error al editar la tarea.' : 'Error al crear la tarea. Por favor, intente nuevamente.'),
          'error'
        );
      }
    } catch (error) {
  ...
  return (
    <div className={modo === 'modal' ? 'p-2' : 'max-w-xl mx-auto mt-8 p-6 bg-white rounded-lg shadow-md border'}>
      ...
      <div className="flex space-x-4 mt-8 justify-end">
        <button 
          ...
          onClick={() => { limpiarFormulario(); if (onCancel) onCancel(); }}
