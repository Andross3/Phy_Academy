...
export default function EstudianteHome() {
  ...
  // NUEVO: Estado para cursos totales
  ...
  const inscribirse = async (idCurso) => {
    ...
    if (res.ok) {
      ...
      // Aquí podrías actualizar "Mis cursos" si lo deseas en el futuro
    } else {
      ...
      <div className="text-center z-10 relative">
        ...
        <div className="mb-10">
          ...
          <div className="flex flex-wrap gap-6 justify-center">
            {misCursos.length === 0 ? (
              <div className="col-span-full text-gray-500 text-center">No hay cursos inscritos.</div>
            ) : (
              misCursos.map((curso, index) => (
                <div
                  key={curso.id}
                  className="bg-gray-900 rounded-lg p-5 w-full max-w-xs shadow-lg border border-gray-800 flex flex-col hover:scale-105 transition-transform duration-200 cursor-pointer"
                  onClick={() => navigate(`/paginaTemaEstudiante/${curso.id}`)}
                >
                  <div className="mb-3">
